...
class Insect:
    ...
    def __init__(self, health, place: Type[Place] | None = None):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                raise Exception("Too many ants in {0}".format(place))
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        reflective_damage_amount = (
            amount + self.damage if self.health - amount <= 0 else amount
        )
        for bee in self.place.bees[:]:
            bee.reduce_health(reflective_damage_amount)
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        if ant := self.ant_contained:
            return ant.action(gamestate)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while place is not None and place.ant is not None:
            if place.ant.id not in self.double_damage_ants:
                place.ant.double()
                if place.ant.is_container and place.ant.ant_contained is not None:
                    place.ant.ant_contained.double()
                self.double_damage_ants.append(place.ant.id)
            place = place.exit

    def reduce_health(self, amount):
        """Reduce health by AMOUNT, and if the QueenAn has no health
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not place.is_hive:
            # add ants excluding the lazerant himself
    ...
    def calculate_damage(self, distance):
        handicap1 = distance * 0.25
        handicap2 = self.insects_shot * 0.0625
        return self.damage - (handicap1 + handicap2)
